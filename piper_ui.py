...
def _get_voice_and_config(req_or_text) -> tuple:
    """Shared voice loading logic. Accepts SynthesizeRequest or (text, voice, speaker_id, ...)."""
...
async def synthesize_stream(
    ...
    """Stream raw PCM chunks as they're generated. First line is a JSON header with sample_rate, sample_width, channels."""
...
LLM_URL = os.getenv("LLM_URL", "http://192.168.1.16:11434")
LLM_MODEL = os.getenv("LLM_MODEL", "llama3")


def _llm_stream(prompt: str):
    """Stream text chunks from Ollama LLM. Yields text deltas."""
    try:
        resp = requests.post(
...
def _pipeline_synthesize(text: str):
    """Generator that yields audio PCM bytes as they're produced. Each yield is one chunk."""
    if not text.strip():
        return
    try:
...
def _run_agent(goal: str) -> str:
    """Run the agent executor for a given goal and return the summary."""
    try:
...
async def agent_endpoint(req: SynthesizeRequest) -> dict:
    """Run the agent planner+executor on a goal and return the result."""
    goal = req.text.strip()
    if not goal:
...
async def voice_pipeline(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            # Check for agent trigger (e.g. "do research quantum computing")
            ...
            # 2. Stream LLM -> TTS in pipeline
            for llm_chunk in _llm_stream(data):
                full_text += llm_chunk
                tts_buffer += llm_chunk

                # Send text to browser as it arrives
                await websocket.send_json({"type": "text", "data": llm_chunk})

                # When we have a sentence or enough text, start TTS
            ...
            # 3. Synthesize any remaining buffered text
            if tts_buffer.strip():
                await websocket.send_json({"type": "status", "state": "speaking"})
